...
module.exports = attr = async (data) => {
   ...
   * @param {Boolean} replying
   */
  CXD.sendFileFromUrl = async (from, url, captions, replying) => {
    var type = ''
    var res = await axios.head(file)
    var mimee = res.headers['content-type']
    type = mimee.split("/")[0]
  ...
  CXD.SendTextWithMentions = (teks, memberr, options = {}, id) => {
    ...
    (id == null || id == undefined || id == false) ? CXD.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : CXD.sendMessage(from, {
      text: teks.trim(),
      contextInfo: {
